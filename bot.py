...
def products_keyboard():
    ...
    for i in range(0, len(items), 3):
        rows.append([product_button(product_id, product)
...
def show_product(api, chat_id, product_id):
    ...
    if product.get("price") is not None:
        price_text = f"{product['price']} {product.get('currency', 'ر.س')}"
    else:
        price_text = "سيتم تحديده"
...
def handle_action(api, chat_id, action):
    if action == "home":
        ...
        show_product(api, chat_id, action.split(":", 1)[1])
...
def main():
    ...
    me = api.call("getMe")
    if not me:
        raise RuntimeError("Unable to connect to Telegram.")
    print(f"Bot @{me.get('username')} is running...")
    offset = 0
    while True:
        try:
