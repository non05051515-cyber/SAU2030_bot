...
def show_payment_methods(api,cid,pid):
    bank_button={"text":"تحويل بنكي — الراجحي","callback_data":f"paybank:{pid}","icon_custom_emoji_id":"5452024291472196683"}
...
def show_bank(api,cid,pid):
    ...
    send_message(api,cid,text,{"inline_keyboard":[[button("✅ تم التحويل",f"receipt:bank:{pid}")],[button("↩️ طرق الدفع",f"buy:{pid}")]]})
def show_bybit(api,cid,pid):
    send_message(api,cid,"🪙 <b>USDT — Bybit</b>\n\nاختر طريقة إرسال USDT:",{"inline_keyboard":[[button("🟡 Bybit Pay",f"bybitid:{pid}")],[button("🔴 USDT • TRON (TRC20)",f"trc20:{pid}")],[button("🟡 USDT • BSC (BEP20)",f"bep20:{pid}")],[button("↩️ طرق الدفع",f"buy:{pid}")]]})
...
def request_receipt(api,cid,pid,method):
    PENDING_RECEIPTS[cid]={"product":pid,"method":method};send_message(api,cid,"📸 <b>إرسال إثبات الدفع</b>\n\nأرسل الآن صورة إيصال/إثبات التحويل هنا في المحادثة.\nسيتم إرسالها مباشرة للإدارة للمراجعة.",{"inline_keyboard":[[button("❌ إلغاء",f"buy:{pid}")]]})
def handle_receipt(api,m):
    cid=m["chat"]["id"];pending=PENDING_RECEIPTS.get(cid)
    if not pending:return False
    if not m.get("photo"):
        send_message(api,cid,"📸 فضلاً أرسل <b>صورة</b> إثبات الدفع.");return True
    method_names={"bank":"الراجحي","bybitid":"Bybit Pay","trc20":"USDT TRC20","bep20":"USDT BEP20"};user=m.get("from",{});username=("@"+user["username"]) if user.get("username") else "بدون معرف"
    send_message(api,ADMIN_ID,f"🧾 <b>إثبات دفع جديد</b>\n\n🛒 المنتج: <b>{order_name(pending['product'])}</b>\n💳 الطريقة: <b>{method_names.get(pending['method'],pending['method'])}</b>\n👤 العميل: {username}\n🆔 ID: <code>{cid}</code>")
    api.call("forwardMessage",chat_id=ADMIN_ID,from_chat_id=cid,message_id=m["message_id"]);PENDING_RECEIPTS.pop(cid,None)
    send_message(api,cid,"✅ <b>تم استلام إثبات الدفع</b>\n\nتم إرسال الإثبات للإدارة للمراجعة. سيتم متابعة طلبك بعد التحقق.",home_keyboard());return True
...
def handle_action(api,cid,action):
    ...
    elif action.startswith("receipt:"):
        _,method,pid=action.split(":",2);request_receipt(api,cid,pid,method)
